...
add_action( 'generate_rewrite_rules', 'register_communities_rewrite_rules' );
function register_communities_rewrite_rules( $wp_rewrite ) {
	$new_rules = array(
...
function fix_communities_subcategory_query($query) {
...
add_filter( 'request', 'fix_communities_subcategory_query', 10 );
...
function filter_communities_post_link( $permalink, $post, $leavename ) {
	if ( get_post_type( $post->ID ) == 'communities' ) {
		$terms = wp_get_post_terms( $post->ID, 'metro-area' );
		$term = ( $terms ) ? $terms[0]->term_id : false;
		$permalink = communities_article_permalink( $post->ID, $term, $leavename, true );
	}
	return $permalink;
}
add_filter( 'post_type_link', 'filter_communities_post_link', 100, 3 );

function filter_communities_section_terms_link( $termlink, $term, $taxonomy = false ) {
...
add_filter( 'term_link', 'filter_communities_section_terms_link', 100, 3 );
