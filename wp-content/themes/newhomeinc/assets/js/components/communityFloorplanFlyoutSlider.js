	...
	init() {
		...
		( function( $ ) {
			function communityFloorplanSliderout() {
				const communityFloorplanElement = document.getElementById( 'detail-page-cta' );
				const communityFloorplanPostId = communityFloorplanElement.getAttribute( 'data-postid' );
				...
				const communityFloorplanData = getUrl + '/wp-json/wp/v2/floorplan/' + communityFloorplanPostId;
				...
				$.ajax( {
					...
					success( res ) {
						...
						postTilte.innerText = res.acf.plan_location_plan_name;
						// eslint-disable-next-line no-console
						console.log( res );
...
export default communityFloorplanFlyoutSlider;
