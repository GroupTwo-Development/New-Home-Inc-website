				...
				$.ajax( {
					...
					success( res ) {
						...
						if ( res.acf.nhi_tour !== '' ) {
							...
							// }
							CommunityFlyoutLinks += `<li class="nhi-tour"><a href="${ res.acf.nhi_tour }" target="_blank"><i class="fas fa-user"></i> <span class="link-title">${ res.acf.nhi_tour_text !== undefined ? res.acf.nhi_tour_text : 'Schedule a Meeting' }</span></a></li>`;
						}
						...
						const getFirectionText = `<span class="cta-location-area">${ res.acf.subdivision_google_map.city }, ${ res.acf.subdivision_google_map.state }, ${ res.acf.subdivision_google_map.post_code }</span>`;
						getDirectionDiv.innerHTML = getFirectionText;
					},
				} );
			}
...
export default communityFlyoutSlider;
